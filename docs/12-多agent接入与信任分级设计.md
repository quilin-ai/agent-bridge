...
### 1.4 Codex 独立身份/token（已实现 + 激活路径）
- Codex 需要自己的 broker 身份（`agentId`，type=codex）+ token + room membership——**绝不复用 Claude 的 token**（broker 允许同 identity 多 socket，复用会让两者变成同一身份，违背 distinct identities）。
- 签发（broker 机，复用现有原语）：`abg auth issue --id codex@<host>`（broker 签）+ `abg room invite <room> codex@<host>`（addMember）。
- **激活（edge 机，已 ship）**：`readAuthToken(dbPath, "codex")` 解析顺序 = env `AGENTBRIDGE_CODEX_TOKEN` > `<collabDir>/auth-token-codex`（collab-store.ts，我独占）。**env 是当前 shipped 激活路径**（daemon 启动前 export 即可），无需碰 cli/*。
- **follow-up（与 aikey 协调）**：`abg auth login --token <T> --agent codex` 写 `auth-token-codex` 的 CLI sugar 在 cli/auth.ts（aikey 单主），作小 follow-up，不阻塞 ① 端到端激活。
- fail-inert：无 env 且无 auth-token-codex → Codex bridge 直接 INERT，Claude bridge + v1 流不受影响。
